...
error_log("DEBUG REGISTER: Generating invite codes for new user");

$codes_generated = 0;
for ($i = 0; $i < 3; $i++) {
    $code_attempts = 0;
    while ($code_attempts < 50) {
        ...
        if (!$check) {
            error_log("DEBUG REGISTER: Failed to prepare code check: " . $conn->error);
            break;
        }
        ...
        if ($check_result->num_rows == 0) {
            ...
            // Insert unique code
            $insert_code = $conn->prepare("INSERT INTO invite_codes (code, owner_user_id) VALUES (?, ?)");
            if ($insert_code) {
                $insert_code->bind_param("si", $code, $new_user_id);
                if ($insert_code->execute()) {
                    $codes_generated++;
                    error_log("DEBUG REGISTER: Generated invite code: $code");
                }
                $insert_code->close();
            }
            break;
...
error_log("DEBUG REGISTER: Generated $codes_generated invite codes");
