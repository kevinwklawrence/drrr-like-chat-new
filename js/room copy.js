$(document).ready(function() {
    ...
    debugLog('room.js loaded, roomId:', roomId);
    ...
    function loadBannedUsers() {
        ...
        $.ajax({
            ...
            data: { room_id: roomId },
            success: function(response) {
                debugLog('Raw banned users response:', response);
                debugLog('Response type:', typeof response);
                debugLog('Is array:', Array.isArray(response));
                debugLog('Response length:', response ? response.length : 'null/undefined');
                ...
                if (typeof response === 'string') {
                    try {
                        ...
                        debugLog('Parsed response:', bannedUsers);
                    } catch (e) {
                ...
                if (!Array.isArray(bannedUsers)) {
                    ...
                    debugLog('Processing', bannedUsers.length, 'banned users');
                    
                    if (bannedUsers.length === 0) {
                        html = '<p class="text-muted">No banned users.</p>';
                        debugLog('No banned users found');
                    } else {
                        debugLog('Found banned users, processing...');
                        
                        bannedUsers.forEach((ban, index) => {
                            debugLog('Processing ban', index + 1, ':', ban);
                            ...
                            debugLog('Ban details - Name:', name, 'Type:', banType, 'Expiry:', expiry, 'Reason:', reason);
    ...
    window.deleteRoom = function() {
        if (confirm('Are you sure you want to delete this room? This action cannot be undone.')) {
            $.ajax({
                ...
                success: function(response) {
                    debugLog('Response from deleteRoom:', response);
                    try {
                        let res = JSON.parse(response);
                        if (res.status === 'success') {
    ...
    window.transferHost = function() {
        ...
        if (confirm('Are you sure you want to transfer host privileges and leave the room?')) {
            $.ajax({
                ...
                success: function(response) {
                    debugLog('Response from transferHost:', response);
                    try {
                        let res = JSON.parse(response);
                        if (res.status === 'success') {
    ...
    // Initialization
    debugLog('Initializing room functions for roomId:', roomId);
    loadMessages();
    loadUsers();
