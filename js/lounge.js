                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h5 class="mb-1">
                                    ...
                                    ${allowsKnocking ? '<i class="fas fa-hand-paper ms-1"></i>' : ''}
                                </h5>
...
// Function to check for knocks (only if user is in a room as host)
function checkForKnocks() {
    // Only check if user might be a host - we'll let the server determine this
    $.ajax({
        ...
        error: function(xhr, status, error) {
            // Silently fail - user might not be a host or in a room
...
function displayKnockNotifications(knocks) {
    knocks.forEach(knock => {
        ...
        const notificationHtml = `
            <div class="alert alert-info knock-notification" id="knock-${knock.id}" role="alert">
        ...
        success: function(result) {
            if (result.status === 'success') {
                ...
                if (response === 'accepted') {
                    ...
                    console.log('Knock accepted');
                } else {
                    console.log('Knock denied');
                }
            } else {
