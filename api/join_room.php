...
error_log("JOIN_ROOM_WITH_COLOR: Starting - room_id: $room_id, user: $user_id_string, password_provided: " . (!empty($password) ? 'YES' : 'NO'));
...
try {
    ...
    if ($ban_check_result['banned']) {
        error_log("JOIN_ROOM_WITH_COLOR: User $user_id_string is banned from room $room_id");
    ...
    $stmt = $conn->prepare("SELECT id, name, capacity, password, has_password FROM chatrooms WHERE id = ?");
    if (!$stmt) {
    ...
    // Handle password protection (existing logic)
    ...
    // Build dynamic INSERT statement with color support
    ...
    // Add color field if it exists in the table and user has a color
    if (in_array('color', $available_columns)) {
        $user_color = $user_session['color'] ?? 'blue';
        $insert_fields[] = 'color';
        $insert_values[] = '?';
        $param_types .= 's';
        $param_values[] = $user_color;
        error_log("JOIN_ROOM_WITH_COLOR: Adding user color: $user_color for user: $user_id_string");
    } else {
        error_log("JOIN_ROOM_WITH_COLOR: WARNING - color column does not exist in chatroom_users table");
    }
    ...
    if (in_array('last_activity', $available_columns)) {
        $insert_fields[] = 'last_activity';
        $insert_values[] = 'NOW()';
    }
    ...
    error_log("JOIN_ROOM_WITH_COLOR: ✅ Successfully joined room $room_id with color " . ($user_session['color'] ?? 'default'));
    echo json_encode([
        'status' => 'success', 
    ...
    error_log("JOIN_ROOM_WITH_COLOR: ❌ Exception - " . $e->getMessage());
