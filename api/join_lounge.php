...
$avatar = $_POST['avatar'] ?? '';
$color = $_POST['color'] ?? 'blue'; // ADDED: Handle color selection
...
try {
    $stmt = $conn->prepare("INSERT INTO global_users (user_id_string, username, guest_name, avatar, guest_avatar, color, is_admin, ip_address) VALUES (?, NULL, ?, ?, ?, ?, 0, ?) ON DUPLICATE KEY UPDATE guest_name = VALUES(guest_name), avatar = VALUES(avatar), guest_avatar = VALUES(guest_avatar), color = VALUES(color), ip_address = VALUES(ip_address), last_activity = CURRENT_TIMESTAMP");
    
    if ($stmt) {
        $stmt->bind_param("ssssss", $guest_user_id, $guest_name, $avatar, $avatar, $color, $ip_address);
        $stmt->execute();
        $stmt->close();
